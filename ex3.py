...
#Initializing the various TCP variants and queuing algorithms in NS-2:
...
def fetchLatency(tcpvar, queuevar, granularity = 0.5):
	...
	for line in lines:
		...
		if(data.time - clock <= granularity):
			...
			output.write(str(clock) + '\t' + str(latency1) + '\t' + str(latency2) + '\n')
			# Clear counter
			clock += granularity
			start_time1 = {}
			start_time2 = {}
...
for tcpvar in TCP_Variant3:
	for q in QUEUE_Variant:
		os.system(ns_command + "ex3.tcl " + tcpvar + " " + q)
